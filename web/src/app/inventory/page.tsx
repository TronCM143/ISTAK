...
export default function InventoryPage() {
  ...
  const [sorting, setSorting] = React.useState<SortingState>([
    { id: "last_borrowed_date", desc: true },
  ]);
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        setAddError(`Failed to add item: ${typeof errData === 'string' ? errData : JSON.stringify(errData)}`);
  ...
  const handleEditItem = async () => {
    ...
    try {
      ...
      const resp = await fetch(`http://192.168.1.17:8000/api/items/${editItem.id}/`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!resp.ok) {
        const errData = await resp.json().catch(() => resp.text());
        setAddError(`Failed to update item: ${typeof errData === 'string' ? errData : JSON.stringify(errData)}`);
    ...
    () => [
        ...
        header: ({ table }) => (
          <Checkbox
            ...
            onCheckedChange={(value) => {
              ...
              setSelectedRows(value ? new Set(data.map((item) => item.id)) : new Set());
            }}
          />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          ...
          >
            Item Name
            {column.getIsSorted() && <span className="ml-2 text-xs">{column.getIsSorted()}</span>}
          </div>
        ),
      ...
      },
      {
        ...
        header: ({ column }) => (
          <div className="flex items-center">
            ...
            <Select
              value={column.getFilterValue() as string | undefined || "__all__"}
      ...
      {
        accessorFn: (row) => row.last_borrowed_date ?? undefined,
        id: "last_borrowed_date",
        header: ({ column }) => (
          <div
            className="flex items-center gap-2 cursor-pointer"
            onClick={() => column.toggleSorting()}
      ...
      },
      {
        ...
        cell: (info) => (
          info.getValue() ? (
            <img src={info.getValue()} alt="Item" className="h-12 w-12 object-cover rounded" />
          ) : (
            <div>No Image</div>
          )
        ),
